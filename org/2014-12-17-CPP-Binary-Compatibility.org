...
外一个问题：[[https://techbase.kde.org/Policies/Binary_Compatibility_Issues_With_C%252B%252B][二进制兼容性 (Binary Compatibility)]]。摘录一下。
...
简单来说，如果修改了某些个类，但是这些修改并没有改变类实例 (class instances) 的大小，同时也没有改
变类的虛函数表 (Virtual Function Table) ，则该改动通常具有二进制兼容性，反之则
不具备。
