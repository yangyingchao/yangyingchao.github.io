...
1. 未添加任何 node 的表头
#+BEGIN_SRC dot :file images/empty_phead.png :cmdline -Kdot -Tpng
  digraph Name {
...
2. 未加入 list 的 =plist_node=
   ...
   各种指针都指向自己
    ...
    #+BEGIN_SRC dot :file images/pnode_0.png :cmdline -Kdot -Tpng
      digraph Name {
          node [fontsize=12 fontname=Courier style=filled];
          edge[color="#666666" style=dashed arrowhead=open];
          // rankdir=LR;
...
3. 添加一个 =plist_node=
   ...
   #+BEGIN_SRC dot :file images/1node.png :cmdline -Kdot -Tpng
     digraph Name {
...
4. 添加两个不同的 Node:
   ...
   #+BEGIN_SRC dot :file images/2nodes.png :cmdline -Kdot -Tpng
     digraph Name {
         node [fontsize=12 fontname=Courier style=filled shape=record];
         edge[color="#666666" style=dashed arrowhead=open tailclip=false];
         node_plist_head[shape=record label="{<f0> plist_head| {<f1>prev(points:node_1)|<f2>next((points:node_1))}}"];
   ...
   5. 添加三个 Node，其中有两个 prio 相同:
      ...
      #+BEGIN_SRC dot :file images/3nodes.png :cmdline -Kdot -Tpng
        digraph Name {
            node [fontsize=12 fontname=Courier style=filled];
            edge[color="#666666" style=dashed arrowhead=open];
            // rankdir=LR;
